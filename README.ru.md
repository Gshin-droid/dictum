...
**В меню:** запись, расшифровка аудиофайла, **Язык и модель**, «Скопировать
последнюю диктовку», «Настройки…», справка, «О программе», выход.

- **Язык и модель** — переключение между тремя моделями. Незнакомая скачается
  сама, выбор запомнится. Не подошла — вернуться тем же меню. Оставлено в меню
  нарочно: это самая частая настройка, ради неё окно открывать незачем.

**В окне настроек** — остальное. Меняется сразу, кнопки «Сохранить» нет: так же
работали прежние пункты меню, и заводить второй порядок ради окна незачем.
