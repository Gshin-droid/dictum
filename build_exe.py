...
def make_icon(path: Path) -> None:
    ...
    Тот же жетон, что у окон программы: тёмный микрофон в светлом круге с
    тёмным ободом. Задача у файла та же самая — он запекается один раз и живёт
    на всех фонах сразу: в светлом проводнике, на тёмной панели задач, на чужом
    рабочем столе. Одноцветный значок обязан этот фон угадать, и любой выбор
    здесь наполовину неверен: чёрный теряется на тёмном, белый — на светлом.

    До этого брался акцентный синий — он читался на обоих фонах, но был третьим
    лицом программы: микрофон в лотке, светлый значок в окнах, синий у файла.
    Жетон снимает и вопрос с фоном, и разнобой: лицо теперь одно.

    Размера 256 в списке нет намеренно. Он там стоял, но в файл не попадал
    никогда: рисунок у нас 64 точки, а PIL размеры крупнее исходника молча
    пропускает — отказа нет, записи тоже. Проверено чтением готового .ico:
    внутри 16, 32, 48 и 64. Просить то, чего не будет, — врать самим себе.

    ponytail: крупные значки в проводнике («огромные») поэтому берут 64 и
    растягивают. Понадобится чёткий 256 — рисовать жетон сразу в 256, а не
    дописывать размер в этот список.
    """
