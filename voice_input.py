...
def badge_image(mic: str = "#1c1c1e", disc: str = "#f2f2f4", ring: str = "#1c1c1e"):
    """Микрофон в светлом круге с тёмным ободом — значок, читаемый на любом фоне.

    Одноцветный значок обязан угадать фон под собой: светлый пропадает на
    светлой теме, тёмный — на тёмной. Значку окон угадывать нечем: рамка окна у
    программы всегда тёмная, а кнопка на панели задач красится по теме системы,
    и это два разных фона у одного файла.

    Жетон снимает вопрос вовсе: светлый круг даёт контраст тёмному фону, тёмный
    обод — светлому. Собственный фон значок несёт с собой, и угадывать больше
    нечего.

    Микрофон не перерисовывается: берётся tray_image и вкладывается внутрь
    уменьшенным. Рисунок у программы один, и правка формы микрофона доходит
    сюда сама.
    """
    from PIL import Image, ImageDraw
    ...
    img = Image.new("RGBA", (64, 64), (0, 0, 0, 0))
    d = ImageDraw.Draw(img)
    d.ellipse([0, 0, 63, 63], fill=ring)    # обод: виден на светлом фоне
    d.ellipse([3, 3, 60, 60], fill=disc)    # круг: даёт контраст на тёмном

    # Пропорции выбраны сравнением отрисовок на 16, 24, 32, 48 и 64 точках, на
    # светлом фоне и на тёмном. Обод тонкий (3 из 64), микрофон крупный (48 из
    # 64): на мелких размерах место решает всё, и толстый обод отнимал его у
    # самого рисунка — микрофон превращался в пятно. Плата известна: на 16
    # точках тонкий обод местами прерывается при уменьшении.
    внутри = tray_image(mic).resize((48, 48), Image.LANCZOS)
    img.paste(внутри, (8, 8), внутри)
    return img
