...
Умеет и второе: **расшифровать готовую запись** — бросил на программу файл,
рядом лёг текст. Час записи разбирает примерно за восемь минут.
...
Два способа, оба делают одно и то же:

- **перетащить файл на `dictum.exe`** мышкой — работает и на ярлык;
- **правый клик по значку у часов → «Расшифровать аудиофайл…»** — откроется
  выбор файла, можно отметить сразу несколько.
