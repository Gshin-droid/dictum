...
def окно():
    ...
    # Корень мы спрятали, а окно настроек — отдельный Toplevel, и оно вылезало
    # на экран по-настоящему: восемь тестов подряд создают и рушат его, человек
    # за той же машиной видит стробоскоп поверх своей работы. Нажатия проходят
    # и на скрытом окне — виду тесты всё равно не судьи.
    win.withdraw()
...
def test_znaki_prepinaniya_pryachutsya(окно):
    ...
    второе.withdraw()  # оно тоже настоящее и тоже мелькало бы на экране
    подписи = []
...
def test_okno_v_testah_ne_vylezaet_na_ekran(окно):
    """Проверки не имеют права мелькать поверх чужой работы.

    Восемь тестов подряд поднимали настоящее окно настроек и рушили его — на
    экране это стробоскоп: окно появляется и исчезает быстрее, чем читается.
    Пользователь увидел это 10.09.2026 посреди своей работы и принял за поломку
    программы. Проверка замкнута на самом результате: окно либо показано, либо нет.
    """
    win, _, _ = окно
    win.update_idletasks()
    assert not win.winfo_ismapped(), "окно настроек показалось на экране во время тестов"
