...
def test_zaglavnaya_posle_tochki():
    """Модель ставит точку, но следующее слово оставляет строчным.

    Вскрылось на живой диктовке: «...заново их подключил. сейчас осуществляю».
    Два выхода модели разошлись — знак поставлен, конец предложения не отмечен.
    """
    assert pn.capitalize_sentences("раз. два! три? четыре") == "раз. Два! Три? Четыре"
    assert pn.capitalize_sentences("подключил.  сейчас") == "подключил.  Сейчас"
    assert pn.capitalize_sentences("рахмет. мен келдім") == "рахмет. Мен келдім"


def test_zaglavnaya_ne_lezet_kuda_ne_prosili():
    """Запятая предложения не кончает, а слово внутри строки трогать нельзя."""
    assert pn.capitalize_sentences("раз, два") == "раз, два"
    assert pn.capitalize_sentences("раз.два") == "раз.два", "без пробела это не граница"
    assert pn.capitalize_sentences("") == ""
    assert pn.capitalize_sentences("раз. 5 мая") == "раз. 5 мая", "цифру заглавить нельзя"


@skip_bez_vesov
def test_zaglavnaya_posle_tochki_na_zhivom_tekste():
    """То же, но через всю цепочку: модель, сборка строки, обе проверки."""
    p = pn.load(ROOT / "models")
    # Текст живой диктовки целиком: на нём модель и ставила точку, оставляя
    # следующее слово строчным. На отрывке покороче она обходится запятыми, и
    # проверять на нём нечего.
    текст = ("так проверка работы микрофона диктую через ну микрофон я перепроверил "
             "все штекеры заново их подключил сейчас осуществляю голосовой ввод "
             "проверяю модель проверяю аппаратуру микрофон проверяю саму программу "
             "диктую достаточно долго чтобы посмотреть не произойдет ли снова отвала "
             "во время диктовки")
    got = p.apply(текст)
    предложения = [s.strip() for s in re.findall(r"[^.!?]+[.!?]", got)]
    assert len(предложения) > 1, f"модель не разбила текст на предложения: {got}"
    for предложение in предложения:
        assert предложение[0].isupper(), f"предложение со строчной: {предложение!r} в {got!r}"


@skip_bez_vesov
def test_kazahskiy_vopros_poluchaet_znak():
    """«ма», «па» — вопросительные частицы казахского. Знак должен встать после них."""
    p = pn.load(ROOT / "models")
    assert p.apply("сен қайда барасың") == "Сен қайда барасың?"
    assert p.apply("бұл дұрыс па") == "Бұл дұрыс па?"


@skip_bez_vesov
def test_kosvennyy_vopros_ne_razryvaetsya_znakom():
    """«...умеет ли, это и проверим» — одно предложение, а не два.

    Прежняя модель ставила здесь вопросительный знак и разрывала фразу пополам,
    и тест это закреплял. Новая ставит запятую — так вопрос остаётся косвенным.
    Носителем языка не выверено: стоит в очереди на вычитку вместе с переводом.
    """
    p = pn.load(ROOT / "models")
